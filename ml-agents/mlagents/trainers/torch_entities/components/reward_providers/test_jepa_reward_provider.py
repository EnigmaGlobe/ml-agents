...
from mlagents.trainers.torch_entities.components.reward_providers.jepa_reward_provider import (
    JepaRewardProvider,
)
...
def make_dummy_buffer(batch_size=2, T=6, S=4, D=8):
    ...
    buf[BufferKey.OBSERVATIONS] = [slots]
    # agent ids length
    buf[BufferKey.AGENT_IDS] = list(range(batch_size))
    return buf


def test_jepa_evaluate_preextracted_slots():
    ...
    provider = JepaRewardProvider(None, cfg)
...
def test_jepa_update_stub_noop():
    buf = make_dummy_buffer()
    class Cfg:
        history_size = 3
        num_preds = 1
